...
const CustomInput = props => {
  ...
  if (props.inputType === "textarea") {
    return <textarea autoFocus {...newProps} />;
  } else {
    return (
      <input
        autoFocus
...
class FieldItem extends React.Component {
  state = {
    ...
    availablepickListValues:
      this.props.inputType === "select" ? this.props.pickListValues : []
  };
  ...
  handleFieldSearch = value => {
    ...
    this.setState({
      availablepickListValues: this.props.pickListValues.filter(aField =>
        aField.label.match(re)
      )
    });
  };
  ...
  render() {
    ...
    let fieldJSX = (
      <div
        ...
        onClick={() =>
          this.setState({ editing: true, fieldValue: this.props.fieldValue })
        }
      >
        <span className="field-item-js" style={{ backgroundColor: "white" }}>
    ...
    if (this.state.editing) {
      if (this.props.inputType === "select") {
        if (this.state.fieldValue.length > 50) {
          ...
          const options = this.state.availablepickListValues.map(aField => (
            <Option key={aField.key} value={aField.key}>
              {aField.label}
            </Option>
          ));
          fieldJSX = (
