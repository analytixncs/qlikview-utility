...
const getApplicationVariables = appName => {
  return readFilePromise(getLocalFile(VAR_FILE)).then(data => {
    ...
    let applicationVars = qvVars.filter(
      qvVar => qvVar.application.toLowerCase() === appName
    );
  ...
  return readFilePromise(getLocalFile(VAR_FILE)).then(data => {
    ...
    fs.writeFile(getLocalFile(VAR_FILE), JSON.stringify(variables), () => {
      ...
      let applicationVars = variables.filter(
        qvVar =>
          qvVar.application.toLowerCase() === varToAdd.application.toLowerCase()
      );
...
const getApplicationData = () => {
  return readFilePromise(getLocalFile(APPNAMES_FILE)).then(data => {
    return JSON.parse(data);
  });
};
//---------------------------------------------------
...
const addAppName = appNameToAdd => {
  return readFilePromise(getLocalFile(APPNAMES_FILE)).then(data => {
    ...
    const dupAppName =
      appNames.filter(name => name.appName === appNameToAdd).length > 0;
    if (dupAppName) {
...
const updateAppName = appToUpdate => {
  return readFilePromise(getLocalFile(APPNAMES_FILE)).then(data => {
    ...
    const oldAppName = appNames.filter(
      appname => appname.id === appToUpdate.id
    )[0].appName;
    ...
    return readFilePromise(getLocalFile(VAR_FILE)).then(data => {
      ...
      return writeFilePromise(
        getLocalFile(VAR_FILE),
        JSON.stringify(newVariablesArray)
      ).then(() => {
        return writeFilePromise(
          getLocalFile(APPNAMES_FILE),
          JSON.stringify(appNames)
...
const getXMLData = appName => {
  return getApplicationVariables(appName).then(applicationVars => {
